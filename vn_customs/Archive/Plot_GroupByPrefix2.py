...
gd.addStringField("Group Column:", "Label", 20)  # Add input for optional prefix filter
...
gd.addStringField("Filter prefix (optional):", "", 20)  # Add input for optional prefix filter
gd.showDialog()
...
unique_prefixes = sorted(unique_prefixes)  # Sort the prefixes for easier selection

## Ensure we have enough unique prefixes to satisfy the number of groups
#if len(unique_prefixes) < num_groups:
#    print("Not enough unique prefixes available after filtering.")
#    exit()
...
for i in range(num_groups):
    default_prefix = unique_prefixes[i] if i < len(unique_prefixes) else unique_prefixes[0]
    gd_groups.addChoice("Prefix for group %d:" % (i + 1), unique_prefixes, default_prefix)
...
# Prepare to plot
x_labels = ["%s (%d)" % (prefix, len(grouped_data[prefix])) for prefix in selected_prefixes]
...
minY = min(min(grouped_data[prefix]) for prefix in selected_prefixes if grouped_data[prefix])
maxY = max(max(grouped_data[prefix]) for prefix in selected_prefixes if grouped_data[prefix])
...
plot.setLimits(0, (len(selected_prefixes) + 1) * group_spacing, minY, maxY)
plot.show()
